...
class Rate extends BE_Controller {
	...
	function save() {
		$response = save_data('tbl_rate',post(),post(':validation'));
		...
		foreach($jml as $i => $k) {
			if($i==0) {
				for($j = 2; $j <= $k; $j++) {
					...
					$save = insert_data('tbl_rate',$data);
					if($save) $c++;
				}
			}
		}
