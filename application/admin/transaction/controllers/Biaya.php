    ...
    function index($p1="") { 
     ...
     function data ($anggaran="", $cabang=""){
        ...
        $coa = get_data('tbl_m_coa a',[
            'select' => 'd.*, '.$select.',b.TOT_'.$cabang.', c.*, '.$select2,
            'where' => "
        ...
        $coaB = get_data('tbl_m_coa a',[
            'select' => 'd.*,'.$selectB.',b.TOT_'.$cabang.', c.*, '.$select2B,
            'where' => "
                a.is_active = '1' and $or_neraca
                ",
            'order_by' => 'a.id',
        ...
        $coaC = get_data('tbl_m_coa a',[
            'select' => 'd.*,'.$selectC.',b.TOT_'.$cabang.', c.*, '.$select2C,
            'where' => "
                a.is_active = '1' and $or_neraca
                ",
            'order_by' => 'a.id',
        ...
        $coaD = get_data('tbl_m_coa a',[
            'select' => 'd.*,'.$selectD.',b.TOT_'.$cabang.', c.*, '.$select2D,
            'where' => "
                a.is_active = '1' and $or_neraca
                ",
            'order_by' => 'a.id',
        ...
        $coaE = get_data('tbl_m_coa a',[
            'select' => 'd.*, '.$selectE.',b.TOT_'.$cabang.', c.*, '.$select2E,
            'where' => "
                a.is_active = '1' and $or_neraca
                ",
            'order_by' => 'a.id',
            'join' => ["$tbl_history b on and a.glwnco = b.glwnco type left","tbl_indek_besaran_biaya c on c.coa = a.glwnco type left","tbl_biaya d on d.glwnco = a.glwnco type left"],
             'where'     => " b.bulan not in(0) and a.glwnco like '580%' group by b.account_name",
        ])->result();
