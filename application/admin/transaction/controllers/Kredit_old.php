        ...
        if(isset($ccore->status_plan)){
            foreach ($tcore as $g => $v_) {
                ...
                if(table_exists($tabel)) {
                    ...
                    $data_view['bln_anggaran'] = $bln_anggaran;

                    if($bln_anggaran) {
                        foreach ($bln_anggaran as $bln) {
                            $v_bln = 'B_' . sprintf("%02d", $bln->bulan) ; 
                            if($bln->tahun != $anggaran->tahun_anggaran and $bln->sumber_data ==2) {
                          ...
                          // cari netto untuk ditampilkan sebagai default
                            foreach ($data['B'] as $val) {
                                ...
                                if($val->glwnco == $kd->coa ){                     
                                  ...
                                  if($kd->netto > 0) $vnetto1 = $kd->netto;
        ...
        foreach ($res as $r => $r1) {
            ...
            switch ($r1['table']) {
              ...
              case 'table7';
                    ...
                    update_data('tbl_budget_plan_kredit',$data4,['kode_anggaran'=>$anggaran->kode_anggaran,'kode_cabang'=>$r1['kode_cabang'],'coa'=>$r1['coa'],'tahun_core'=>$r1['tahun']]);
                    break;                            
            } 
