...
class Rko_kebijakan_strategis extends BE_Controller {
    ...
    function index() {
        $cabang_user  = get_data('tbl_user',[
            'where' => [
                'is_active' => 1,
                'id_group'  => id_group_access('rko_kebijakan_strategis')
            ]
        ])->result();
    ...
    function data($anggaran="", $cabang="", $tipe = 'table'){
        ...
        $a = get_access('rko_kebijakan_strategis');
        ...
        $response   = array(
            'table' => $this->load->view('transaction/rko_kebijakan_strategis/table',$data,true),
            'list'  => $list,
        );
    ...
    function save(){
        ...
        if($kebijakan):
            foreach ($kebijakan as $i => $v) {
                ...
                $arrID = array();
                if(post('id')):
                    $dt = get_data('tbl_kebijakan_strategis','id',post('id'))->row();
                endif;
                if($aktivitas):
                ...
                if(count($arrID)>0 && post('id')):
                    delete_data('tbl_kebijakan_strategis',['kode_anggaran'=>$ckode_anggaran,'id not'=>$arrID,'kode_cabang'=>$kode_cabang,'tahun'=>$tahun,'name' => $dt->name]);
                endif;
            }
        else:
            if(post('id')):
