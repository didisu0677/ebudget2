...
class Rko_usulan_kredit extends BE_Controller {
    var $detail_tahun;
    var $kode_anggaran;
    ...
    function index() {
        $cabang_user  = get_data('tbl_user',[
            'where' => [
                'is_active' => 1,
                'id_group'  => id_group_access('rko_usulan_kredit')
            ]
        ])->result();
    ...
    function data($anggaran="", $cabang="", $tipe = 'table') {
        ...
        foreach($data['grup'][0] as $m0) {         
            ...
            $data['produk'][$m0->keterangan.'all']  = get_data('tbl_target_kredit a',$arr)->result_array();
        ...
        $data['detail_tahun']    = $this->detail_tahun;
        $response   = array(
            'table'     => $this->load->view('transaction/rko_usulan_kredit/table',$data,true),
        );
	...
	function get_data() {
        ...
        $data['detail_ket'] = get_data('tbl_target_kredit a',[
            'where' => [
            'kode_anggaran' => $dt->kode_anggaran,    
            'tahun' => $dt->tahun,
            'kode_cabang' => $dt->kode_cabang,
    ...
    function save() {
        ...
        foreach($keterangan as $i => $v) {
            ...
            if(!isset($cek->id)) {
                $id = insert_data('tbl_target_kredit',$c);
            }else{
                $id = $id_data[$i];
        ...
        if(post('id')):
            delete_data('tbl_target_kredit',['kode_anggaran'=>$ckode_anggaran,'keterangan not' =>$keterangan,'kode_cabang'=>$kode_cabang,'tahun'=>$anggaran->tahun_anggaran,'grup'=>$grup_aset]);
        endif;  
