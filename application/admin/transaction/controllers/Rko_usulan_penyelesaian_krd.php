...
class Rko_usulan_penyelesaian_krd extends BE_Controller {
    ...
    function index() {
        $cabang_user  = get_data('tbl_user',[
            'where' => [
                'is_active' => 1,
                'id_group'  => id_group_access('rko_usulan_penyelesaian_krd')
            ]
        ])->result();
        $kode_cabang          = [];
    ...
    function data($anggaran="", $cabang="", $tipe = 'table') {
        ...
        $a = get_access('rko_usulan_penyelesaian_krd');
        ...
        $response	= array(
            'table'		=> $this->load->view('transaction/rko_usulan_penyelesaian_krd/table',$data,true),
        );
