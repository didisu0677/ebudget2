...
class Usulan_dpk extends BE_Controller {
    ...
    function index() {
        $cabang_user  = get_data('tbl_user',[
            'where' => [
                'is_active' => 1,
                'id_group'  => id_group_access('usulan_dpk')
            ]
        ])->result();
    ...
    function data($anggaran="", $cabang="", $tipe = 'table') {
        ...
        $data['detail_tahun']    = $this->detail_tahun;
        $response   = array(
            'table'     => $this->load->view('transaction/usulan_dpk/table',$data,true),
        );
    ...
    function save() {
        ...
        foreach($keterangan as $i => $v) {
            ...
            if(!isset($cek->id)) {
                ...
                $id = update_data('tbl_target_dpk',$c,[
                    'kode_anggaran'   => $ckode_anggaran,
                    'keterangan_anggaran' => $anggaran->keterangan,
                    'kode_cabang'     => $kode_cabang,
                    'tahun'           => $anggaran->tahun_anggaran,
