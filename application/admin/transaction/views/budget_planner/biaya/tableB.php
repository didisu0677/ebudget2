...
$table ='<tr><td class="border-none">.</td></tr><tr><td class="border-none">.</td></tr>
		<tr style="margin-top:30px">
            ...
            <th class="text-center" style="min-width:80px;background-color: #e64a19; color: white !important;">Real '.month_lang($bulan_terakhir).'</th>
        </tr>';
$item = $table;
...
foreach ($coa as $k => $v) {
								...
								foreach ($detail['4'][$v4->glwnco] as $k5 => $v5) {
									...
									for ($i=1; $i <= 12 ; $i++) { 
										if(count($dt6)>0){ 
											...
											$item5 .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right edited" data-name="bulan_b'.$i.'" data-id="'.$v5->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus5).'</div></td>'; 
										} 
	...
	$item .= '<td'.$bd.'></td>';
	$item .= '<td'.$bg.'>'.check_data($val,$minus3).'</td>';
	$item .= '<td'.$bg.'>'.check_data($v->hasil2,$minus2).'</td>';
	$item .= '<td'.$bg.'>'.check_data($v->hasil,$minus2).'</td>';
	$item .= '</tr>';
	$item .= $item2;
}
echo $item;
