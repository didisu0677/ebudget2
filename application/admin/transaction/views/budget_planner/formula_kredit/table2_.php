...
//			debug($v);die;
		...
		$item .= '<td>'.remove_spaces(strtoupper($v->nama_produk_kredit)).'</td>';	
				...
				//	 debug($v);die;
					...
					$item .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right edited" data-name="bulan_'.$v2->bulan.'" data-id="'.$v2->sumber_data.'-'.$v->coa.'" data-value="'.$$T.'">'.custom_format($$T).'</div></td>';
				}
				$item .= '<tr>';
				...
				foreach ($detail_tahun as $k2 => $v2) {
					...
					$item .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right edited" data-name="bulan_'.$v2->bulan.'" data-id="'.$v2->sumber_data.'-'.$v->coa.'" data-value="'.$$XB.'">'.custom_format($$XB).'</div></td>';
				}
				$i_add = 0;
				foreach ($dd as $k3 => $v3) {
					$i_add++;
					...
					foreach ($detail_tahun as $k2 => $v2) {
						...
						$item .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right edited" data-name="bulan_'.$v2->bulan.'" data-id="'.$v2->sumber_data.'-'.$v->coa.'" data-value="'.$$R2.'">'.$$R2.'</div></td>';
					}
				}
