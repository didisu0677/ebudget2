<?php
	...
	foreach ($dd as $k => $v) {
		...
		foreach ($detail_tahun as $k2 => $v2) {
			...
			$item .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right edited" data-name="bulan_'.$v2->bulan.'" data-id="'.$v2->sumber_data.'-'.$code.'" data-value="'.$nv.'">'.$nv.'</div></td>';
		}
		$item .= '</tr>';
	}
	echo $item;
