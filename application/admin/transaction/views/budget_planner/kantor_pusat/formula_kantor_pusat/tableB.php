    ...
    $test = [];
       foreach ($B as $val) {
                    ...
                    if(!empty($E4)){
                        foreach ($E4 as $key) {
                            if($key->bulan == $val2->bulan){
                                ...
                                if($tambahBaruGlwn1622 == 0){
                                    $tambah =  $hasil - ($tambahBaruPenyGlwn + $bulanLama);
        ...
        if($val->glwnco == '1621014'){
            ...
            $item .= '<td class="text-right">'.custom_format($valB[0]['hasil2'] * -1).'</td>';
           
            $item .= '<td class="text-right">'.custom_format($valB[0]['hasil'] * -1).'</td>';
            $bulanLama = ($valB[0]['hasil'] * -1) - ($valB[0]['hasil2'] * -1); 
