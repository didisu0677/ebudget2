...
foreach ($coa as $k => $v) {
		...
		foreach ($detail['1'][$v->glwnco] as $k2 => $v2) {
				...
				foreach ($detail['2'][$v2->glwnco] as $k3 => $v3) {
						...
						foreach ($detail['3'][$v3->glwnco] as $k4 => $v4) {
								...
								foreach ($detail['4'][$v4->glwnco] as $k5 => $v5) {
									...
									if(isset($detail['5'][$v5->glwnco])){
										foreach ($detail['5'][$v5->glwnco] as $k6 => $v6) {
											...
											for ($i=1; $i <=12 ; $i++) {
												...
												$item6 .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right" data-name="'.$v6->{'TOT_'.$cabang}.'" data-id="'.$v6->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus6).'</div></td>';
									...
									for ($i=1; $i <= 12 ; $i++) { 
										if(count($dt6)>0){ 
											...
											$val = $value * $i; 
											$item5.= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right" data-name="'.$v5->{'TOT_'.$cabang}.'" data-id="'.$v5->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus5).'</div></td>';
										}
							...
							for ($i=1; $i <= 12 ; $i++) {
								if(count($dt5)>0){ 
									...
									$val = $value * $i; 
									$item4.= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right" data-name="'.$v4->{'TOT_'.$cabang}.'" data-id="'.$v4->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus4).'</div></td>';
								}
					...
					for ($i=1; $i <= 12 ; $i++) {
						if(count($dt4)>0){ 
							...
							$val = $value * $i; 
							$item3.= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right" data-name="'.$v3->{'TOT_'.$cabang}.'" data-id="'.$v3->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus3).'</div></td>';
						}
			...
			for ($i=1; $i <=12 ; $i++) { 
				if(count($dt3)>0){ 
					...
					$val = $value * $i; 
					$item2.= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right" data-name="'.$v2->{'TOT_'.$cabang}.'" data-id="'.$v2->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus2).'</div></td>';
				}
	...
	for ($i=1; $i <= 12 ; $i++) { 
		if(count($dt2)>0){ 
			...
			$val = $value * $i; 
			$item .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value text-right" data-name="'.$v->{'TOT_'.$cabang}.'" data-id="'.$v->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus).'</div></td>';
		}
