...
modal_open('modal-form','','modal-lg','data-openCallback="formOpen"');
	modal_body();
		form_open(base_url('transaction/rko_usulan_dpk/save'),'post','form'); 
				col_init(2,4);
				input('hidden','id','id');
			...
			<div class="card mb-2">
				<div class="card-header"><?php echo lang('target_dana_pihak_ketiga'); ?></div>
				<div class="card-body">
		            <div class="form-group row">
						<div class="col-md-7 col-9 mb-1 mb-md-0">
							<input type="hidden" name="id_data[]" id="id_data" class="id_data">
...
function getData() {
	...
	var page = base_url + 'transaction/rko_usulan_dpk/data';
...
function save_perubahan() {
	...
	$.ajax({
		url : base_url + 'transaction/rko_usulan_dpk/save_perubahan',
		data 	: {
			'json' : jsonString,
			verifikasi : i
		},
		...
		$.each(response.detail_ket,function(e,d){
			if(e == '0') {
				...
				$('#grup_aset').val(d.grup).trigger('change');
				 // document.getElementById("grup_aset").style.display="none";
			} else {
				add_row_anggota();
				$('#additional-anggota .id_data').last().val(d.id);
				$('#additional-anggota .keterangan').last().val(d.keterangan);
				$('#additional-anggota .grup_aset').last().val(d.grup).trigger('change');	
			}
		});
		$('body').find('.grup_aset').hide();
