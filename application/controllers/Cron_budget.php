...
class Cron_budget extends MY_Controller {
    ...
    function link_cabang() {
        ...
        $dok = get_data('tbl_dokumen_online','is_active',1)->result();
        $nama_cabang = '';
        foreach ($dok as $a) {

            $cabang = get_data('tbl_m_cabang','kode_cabang',$a->kode_cabang)->row();
            if(isset($cabang->nama_cabang)) {
                $nama_cabang = $cabang->nama_cabang;
            }

            update_data('tbl_dokumen_online',['nama_cabang'=>$nama_cabang],['id'=>$a->id,'kode_cabang'=>$a->kode_cabang]);
        }   
        echo 'Success' ;    
    }
